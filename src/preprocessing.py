    ...
    def pretrain_get_data(self, data_size=20000000,min_len=32,max_len=1024, data_name='skt'):# 64/1024
        ...
        if data_name=='skt':
            ...
            if not path.exists(data_path):
                ...
                # qry=f"select * from (select  substr(svc_mgmt_num,1,{sub_token_length}) as token, count(*) as cnt from x1112020.svc_{data_name}_log_list_temp where ym={self.strd_ym} group by substr(svc_mgmt_num,1,{sub_token_length})) as a order by cnt"
                ...
                for i in tqdm(token.values):
                    ...
                    # array<string> 형태의 컬럼을 load_from_hive로 부를 시 양 끝 대괄호가 없어짐.
                    # data_tmp = bq_to_pandas(f"select * from x1112020.svc_{data_name}_log_list_temp where ym={self.strd_ym} and list_len>={min_len} and list_len<={max_len} and substr(svc_mgmt_num,1,{sub_token_length}) in ("+ "'"+f"{i[0]}" + "'"+f") limit {i[2]}")
            ...
            if not path.exists(data_path):
                ...
                ,       array_to_string(array_agg(item),",","") as item -- 봐야할 sequence
                ,       array_to_string(array_agg(cat1),",","") as cat1
                ,       array_to_string(array_agg(cat1),",","") as cat2
                ...
                FROM
                    ...
                    ,       regexp_replace(regexp_replace(regexp_replace(cat1,' ',''),'"',''),',','') as cat1
                    ,       regexp_replace(regexp_replace(regexp_replace(cat1,' ',''),'"',''),',','') as cat2
                    ,       regexp_replace(regexp_replace(regexp_replace(type,' ',''),'"',''),',','') as type
                    ,       unix_time
                    FROM    x1113573.svc_amazon_log_temp
        ...
        if data_name=='skt':    
            ...
            if not path.exists(data_path):
                ...
                for i in columns:
                    ...
                    qry=f'''select regexp_replace(regexp_replace({i},' ',''),'"','') as {i} , count(*) as cnt from x1113573.svc_amazon_log_temp where {i} is not null group by regexp_replace(regexp_replace({i},' ',''),'"','')'''
