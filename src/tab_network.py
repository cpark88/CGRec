...
class TabNetEncoder(torch.nn.Module):
    ...
    def forward(self, x, prior=None):
        ...
        if prior is None:
            prior = torch.ones((bs, self.attention_dim)).to('cuda')
    ...
    def forward_masks(self, x):
        ...
        bs = x.shape[0]  # batch size
        prior = torch.ones((bs, self.attention_dim)).to('cuda')
        M_explain = torch.zeros(x.shape).to('cuda')
        att = self.initial_splitter(x)[:, self.n_d :]
        masks = {}
...
class TabNetDecoder(torch.nn.Module):
    ...
    def forward(self, steps_output):
        ...
        for step_nb, step_output in enumerate(steps_output):
            ...
            x = Tanh()(x)
            res = torch.add(res, x)
        res_final = self.reconstruction_layer(res)
        return res_final, res
...
class TabNetPretraining(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            res, user_embedding = self.decoder(steps_out)
            return res, user_embedding, embedded_x, obfuscated_vars
        else:
            steps_out, _ = self.encoder(embedded_x)
            res, user_embedding = self.decoder(steps_out)
            return res, user_embedding, embedded_x, torch.ones(embedded_x.shape).to('cuda')
...
class GLU_Block(torch.nn.Module):
    ...
    def forward(self, x):
        scale = torch.sqrt(torch.FloatTensor([0.5]).to('cuda'))
...
class EmbeddingGenerator(torch.nn.Module):
    ...
    def __init__(self, input_dim, cat_dims, cat_idxs, cat_emb_dims, group_matrix):
        ...
        if cat_dims == [] and cat_idxs == []:
            ...
            self.embedding_group_matrix = group_matrix.to('cuda')
            return
        else:
        ...
        self.linear_emedding_output = Linear(self.post_embed_dim, self.post_embed_dim, bias=False)
    ...
    def forward(self, x):
        ...
        post_embeddings = self.linear_emedding_output(post_embeddings)
...
class RandomObfuscator(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        obfuscated_groups = torch.bernoulli(
            self.pretraining_ratio * torch.ones((bs, self.num_groups), device='cuda')
        )
